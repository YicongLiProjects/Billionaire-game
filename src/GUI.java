...
public class GUI {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					window.frmBillionaire.setResizable(false);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	private void initialize() {
		...
		playButton.setForeground(new Color(0, 100, 0));
		...
		itemUpgradeBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				moneyAmountUpgradePanel.setText(Long.toString(game.getMoneyAmount()));
			}
		});
		...
		upgradeButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				int confirmUpgrade = JOptionPane.showConfirmDialog(null, "Do you want to upgrade this business?", 
						"Upgrade business", JOptionPane.YES_NO_OPTION);
		...
		endDayButton.setForeground(Color.BLACK);
		...
		endDayButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				"\nYou're on day " + Long.toString(game.getDays() + 1), "Day ended", JOptionPane.INFORMATION_MESSAGE);
				game.endDay();
				if (game.isBillionaire()) {
					cardLayout.show(frmBillionaire.getContentPane(), "gameOverPanel");
					scoreField.setText(Long.toString(game.calculateScore()));
				}
			}
		});
		...
		moneyAmount.setText(Long.toString(game.getMoneyAmount()));
		gamePanel.add(moneyAmount);
		moneyAmount.setColumns(10);
		
		moneySign = new JLabel("$");
		moneySign.setForeground(new Color(0, 128, 0));
		...
		expectedRevenue.setText(Long.toString(game.calculateExpectedRevenue()));
		...
		dayNumberField.setText(Long.toString(game.getDays()));
