...
public final class Game {
	...
	public void setMoneyAmount(long newMoneyValue) {
		this.moneyAmount = newMoneyValue;
	}
	...
	public void endDay() {
		...
		this.gui.getMoneyAmount().setText(Long.toString(moneyAmount));
		// Stats menu
		this.gui.getNbrDaysTextArea().setText(Integer.toString(days) + " days");
		this.gui.getTotalMoneyEarnedTextArea().setText(Long.toString(totalRevenue) + "$");
	 ...
	 */
	public long calculateExpectedRevenue() {
		long expRevenue = 0;
		...
		this.getGui().getExpectedRevenue().setText(Long.toString(expRevenue));
		return expRevenue;
	}
	...
	public boolean buy(Item i) {
		...
		if (i instanceof TemporaryItem) {
			...
			this.gui.getMoneyAmount().setText(Long.toString(moneyAmount));
			this.gui.getTotalMoneySpentTextArea().setText(Long.toString(spentMoney) + "$");
			...
			switch (permanentItem.getName()) {
				case "Book of business":
					...
					long newRevenue = this.calculateExpectedRevenue();
					gui.getExpectedRevenue().setText(Long.toString(newRevenue));
					break;
	...
	public boolean buy(Business b) {
		if (!boughtBusinesses.contains(b) && moneyAmount >= b.getCost()) {
			...
			this.getGui().getMoneyAmount().setText(Long.toString(moneyAmount));
			this.getGui().getTotalMoneySpentTextArea().setText(Long.toString(spentMoney) + "$");
			b.setLevel(1);
			b.setBaseCost((int) (b.getBaseCost() * 1.3));
			b.setCost(this.calculateBusinessUpgradeCost(b));
			this.businessPurchaseMultiplier = 1.0;
			long newRevenue = this.calculateExpectedRevenue();
			this.getGui().getExpectedRevenue().setText(Long.toString(newRevenue));
			return true;
		}
	...
	public void setSpentMoney(long newTotalSpent) {
		this.spentMoney = newTotalSpent;
	}
