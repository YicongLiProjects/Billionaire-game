...
public final class PermanentItem extends Item {
	...
	public boolean upgrade() {
		...
		if (deductMoney) {
			long newMoneyValue = this.getGame().getMoneyAmount() - this.getCost();
			this.getGame().setMoneyAmount(newMoneyValue);
			this.getGame().getGui().getMoneyAmount().setText(Long.toString(newMoneyValue));
			long newTotalSpent = this.getGame().getSpentMoney() + this.getCost();
			this.getGame().setSpentMoney(newTotalSpent);
			this.getGame().getGui().getTotalMoneySpentTextArea().setText(Long.toString(newTotalSpent) + "$");
		...
		switch(this.getName()) {
			case "Book of business":
				...
				long newRevenue = this.getGame().calculateExpectedRevenue();
				this.getGame().getGui().getExpectedRevenue().setText(Long.toString(newRevenue));
				break;
			case "Construction license":
				if (this.level == 4) {
		...
		this.getGame().getGui().getMoneyAmountUpgradePanel().setText(Long.toString(this.getGame().getMoneyAmount()));
		deductMoney = true;
		return true;
	}
	...
	// For testing purposes, can set the level to any Long.
